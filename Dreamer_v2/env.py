...
ATARI_ENVS = ['PongDeterministic-v0']
...
class AtariEnv:
    def __init__(self, env, seed, max_episode_length, action_repeat, bit_depth):
        ...
        self.action_repeat = action_repeat
        self.bit_depth = bit_depth
        self.num_action = self._env.action_space.n
        self.observation_size = (3, 64, 64)
    ...
    def step(self, action: torch.Tensor):
        """action should be a one-hot vector"""
        assert type(action) != int and action.shape[-1] == self.num_action
    ...
    def sample_random_action(self):
        """Sample an action randomly from a uniform distribution over all valid actions
           Returned Action is a one-hot vector
        """
        return torch.from_numpy(np.identity(self.num_action)[self._env.action_space.sample()])
...
def Env(env, symbolic, seed, max_episode_length, action_repeat, bit_depth):
    if env in GYM_ENVS:
        ...
        return AtariEnv(env, seed, max_episode_length, action_repeat, bit_depth)
