...
class ConvEncoder(nn.Module):
    def __init__(self, depth=32, activation=nn.ReLu):
        self.activation = activation
        self.depth = depth
        self.conv1 = nn.Conv2d(3, self.depth, 4, strides=2)
        self.conv2 = nn.Conv2d(3, self.depth * 2, 4, strides=2)
        self.conv3 = nn.Conv2d(3, self.depth * 4, 4, strides=2)
        self.conv4 = nn.Conv2d(3, self.depth * 8, 4, strides=2)

        self.modules = [self.conv1, self.conv2, self.conv3, self.conv4]

    def forward(self, obs):
        B, S, H, W, C = obs['image'].shape
        x = obs['image'].view(B * S, H, W, C)
        x = self.activation(self.conv1(x))
        x = self.activation(self.conv2(x))
        x = self.activation(self.conv3(x))
        x = self.activation(self.conv4(x))
        return x.view(H, W, C, 32 * self.depth)
...
class ConvDecoder(nn.Module):
    def __init__(self, depth=32, activation=nn.ReLu, shape=(64, 64, 3), latent_dim=230):
        self.activation = activation
        self.depth = depth
        self.shape = shape
        self.dense = nn.Linear(latent_dim, 32 * self.depth),
        self.convs = nn.Sequential(
            nn.ReLU(),
            nn.ConvTranspose2d(
                32 * self.depth, 4 * self._depth, 5, strides=2),  # TODO: check num of input
            nn.ReLU(),
            nn.ConvTranspose2d(
                self.depth * 4, 2 * self._depth, 5, strides=2),
            nn.ReLU(),
            nn.ConvTranspose2d(
                self.depth * 2, 1 * self._depth, 6, strides=2),
            nn.ReLU(),
            nn.ConvTranspose2d(
                self.depth * 1, self.shape[-1], 6, strides=2)
        )
        self.modules = [self.dense, self.convs]

    def forward(self, features):
        B, S, latent_dim = features.size()
        x = self.dense(features)
        x = x.view(-1, 1, 1, 32 * self.depth)
        x = self.convs(x)
        loc = x.view(B, S, *self.shape)
        return loc
